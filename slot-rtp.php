...
$date = date('Y-m-d');
$stat = mysqli_query($conn, "INSERT INTO `tb_stat` (`ip`, `date`, `hits`, `page`, `user`) VALUES ('$ip', '$date', 1, 'RTP Slot Pragmatic', '$pengguna')") or die(mysqli_error($conn));
...
<head>
  ...
  <?php include('seo/meta/public-slot-rtp.php'); ?>
...
<body>
  ...
  <div id="wrapper">
    ...
    <h1 class="sr-only">RTP Slot <?= $app_name; ?></h1>
    ...
    <div class="bg-custom d-none d-sm-block pb-3 pt-0">
      <div class="container">
        <ul class="nav" style="justify-content: center;">
          ...
          while ($sp = mysqli_fetch_array($sql_provider)) :
            $provider = $sp['code'];
            $hitungProduk = mysqli_query($conn, "SELECT * FROM `tb_gamenew` WHERE `game_name` = '$provider'") or die(mysqli_error($conn));
            $hp = mysqli_num_rows($hitungProduk);
            if ($hp > 0) :
          ?>
              <li class="nav-item menuprovider">
    ...
    <div class="bg-custom pb-5">
      <div class="container">
        <div class="row">
          <?php
          $sql_3 = mysqli_query($conn, "SELECT * FROM `tb_gamenew` WHERE `game_category` = 'slots' ORDER BY cuid ASC") or die(mysqli_error($conn));
          ...
          ?>
            <div class="col-sm-2 col-4 text-center pt-0 p-2">
              <a href="<?= $playUrl; ?>">
                <img data-sizes="auto" data-src="<?= $s3['game_image']; ?>" src="<?= $urldomain; ?>/assets/images/loading.gif" class="lazyload shadow gamertpimg" alt="Slot <?= $app_name; ?>">
