        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4">
              <span class="text-muted fw-light">Information /</span> <span class="text-muted fw-light">Togel /</span> Pasaran
            </h4>
            <div class="row">
              <div class="col-sm-3">
                <div class="card">
                  <div class="card-body">
                      ...
                      if ($_GET['notif'] == 1) {
                        echo '
                              <div class="alert alert-success d-flex align-items-center" role="alert">
                                ...
                                <span><strong>Well Done!</strong> Pasaran Saved!</span>
                              </div>
                            ';
                      }
                    }
                    if (isset($_GET['catID'])) {
                      $catID = $_GET['catID'];
                      $sql_2 = mysqli_query($conn, "SELECT * FROM `tb_pasaran` WHERE cuid = '$catID'") or die(mysqli_error($conn));
                      $s2 = mysqli_fetch_array($sql_2);
                    }
                    ?>
                    <form role="form" action="<?= $urladmin; ?>/function/add-pasaran.php" method="post" enctype="multipart/form-data">
                      <div class="form-group mb-2">
                        <label class="form-label">Title :</label>
                        <input class="form-control" type="text" name="title" value="<?php if (isset($_GET['catID'])) {
                                                                                      echo $s2['title'];
                                                                                    } ?>" required>
                      ...
                      <div class="form-group mb-2">
                        ...
                        <input class="form-control" type="text" name="kode" value="<?php if (isset($_GET['catID'])) {
                                                                                      echo $s2['code'];
                                                                                    } ?>">
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Open Time :</label>
                        <input class="form-control" type="text" name="time_open" value="<?php if (isset($_GET['catID'])) {
                                                                                          echo $s2['time_open'];
                                                                                        } ?>">
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Close Result :</label>
                        <input class="form-control" type="text" name="close_result" value="<?php if (isset($_GET['catID'])) {
                                                                                              echo $s2['close_result'];
                                                                                            } ?>">
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Time Result :</label>
                        <input class="form-control" type="text" name="time_result" value="<?php if (isset($_GET['catID'])) {
                                                                                            echo $s2['time_result'];
                                                                                          } ?>">
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Step Period :</label>
                        <input class="form-control" type="number" name="step_periode" value="1">
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Website Link :</label>
                        <input class="form-control" type="text" name="website" value="<?php if (isset($_GET['catID'])) {
                                                                                        echo $s2['website'];
                                                                                      } ?>">
                      </div>
                      <div class="form-group mb-2">
                      ...
                      <a href="<?= $urladmin; ?>/pasaran/" class="btn btn-light">Cancel</a>
                    </form>
                  </div>
                </div>
              </div>
                ...
                <div class="card">
                  <div class="card-datatable table-responsive">
                    <table id="default-datatable" class="invoice-list-table table border-top">
                      ...
                      <tbody>
                        <?php
                        $sql_1 = mysqli_query($conn, "SELECT * FROM `tb_pasaran` ORDER BY cuid ASC") or die(mysqli_error($conn));
                        $no = 0;
                        ...
                        ?>
                          <tr>
                            ...
                            <td class="text-left" style="vertical-align: middle; white-space: normal; font-size: 14px;"><?= $s1['code']; ?></td>
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;"><?= $s1['time_open']; ?></td>
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;"><?= $s1['close_result']; ?></td>
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;"><?= $s1['time_result']; ?></td>
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;"><a href="<?= $s1['website']; ?>" target="_blank"><?= $s1['website']; ?></a></td>
                            <td class="text-center" style="vertical-align: middle; white-space: nowrap; font-size: 14px;">
                              <a href="<?= $urladmin; ?>/function/pasaran-status.php?postID=<?= $s1['cuid']; ?>&status=<?= $s1['status']; ?>" class="btn <?php if ($s1['status'] == 0) {
                                                                                                                                                            echo 'btn-success';
                                                                                                                                                          } else {
                                                                                                                                                            echo 'btn-warning';
                                                                                                                                                          } ?> btn-sm"><i class="fa fa-heart"></i></a>
                              <a href="<?= $urladmin; ?>/pasaran/?catID=<?= $s1['cuid']; ?>" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></a>
                              <?php if ($u['level'] == 'superadmin') : ?>
                                <a href="<?= $urladmin; ?>/function/del-pasaran.php?cuid=<?= $s1['cuid']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure want remove this data?');"><i class="fa fa-trash"></i></a>
                              <?php endif; ?>
                            </td>
                          </tr>
                        <?php endwhile; ?>
