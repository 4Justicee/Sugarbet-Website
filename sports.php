...
$date = date('Y-m-d');
$stat = mysqli_query($conn, "INSERT INTO `tb_stat` (`ip`, `date`, `hits`, `page`, `user`) VALUES ('$ip', '$date', 1, 'Sport', '$pengguna')") or die(mysqli_error($conn));
...
<head>
  ...
  <?php include('seo/meta/public-sports.php'); ?>
...
<body>
  ...
  <div id="wrapper">
    ...
    <h1 class="sr-only">Sports <?= $app_name; ?></h1>
    ...
    <div class="bg-custom d-block d-sm-none">
      <div class="mobileprovider">
        <ul class="nav" style="width: 1000px; height: auto;">
          <?php
          $sql_provider = mysqli_query($conn, "SELECT a.*, b.* FROM tb_provider as a INNER JOIN tb_tripayapi as b ON a.providerid = b.provider WHERE a.jenis = 2 ORDER BY a.cuid") or die(mysqli_error($conn));
          while ($sp = mysqli_fetch_array($sql_provider)) {
            $provider = $sp['providerid'];
            $hitungProduk = mysqli_query($conn, "SELECT * FROM `tb_gamelist` WHERE provider = '$provider' AND `datatype` = 'VSB'") or die(mysqli_error($conn));
            $hp = mysqli_num_rows($hitungProduk);
            if ($hp > 0) {
          ?>
              <li class="nav-item menuprovider">
                <a href="<?= $urldomain; ?>/sports/<?= $sp['slug']; ?>/">
      ...
      <div class="container">
        <div class="row">
          ...
          if (isset($_GET['provider'])) {
            ...
            $pageLink = 'sports/' . $provider . '/';
          } else {
            $where = "`provider` = 'PragmaticPlay' AND";
            $pageLink = 'sports/?';
          }

          $sql_3 = mysqli_query($conn, "SELECT * FROM `tb_gamelist` WHERE $where `datatype` IN ('VSB') ORDER BY cuid ASC") or die(mysqli_error($conn));
          ...
          ?>
            <div class="col-sm-2 col-4 text-center pt-0 p-2">
              <a href="<?= $playUrl; ?>" target="_blank">
                <img data-sizes="auto" data-src="<?= $urldomain; ?>/<?= $s3['image']; ?>" src="<?= $urldomain; ?>/assets/images/loading.gif" class="lazyload shadow gamenormalimg" alt="Sports <?= $app_name; ?>">
