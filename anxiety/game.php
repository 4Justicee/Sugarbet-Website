        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4">
              <span class="text-muted fw-light">Information /</span> <span class="text-muted fw-light">Togel /</span> Game
            </h4>
            <div class="row">
              <div class="col-sm-3">
                <div class="card">
                  <div class="card-body">
                      ...
                      if ($_GET['notif'] == 1) {
                        echo '
                              <div class="alert alert-success d-flex align-items-center" role="alert">
                                ...
                                <span><strong>Well Done!</strong> Game Saved!</span>
                              </div>
                            ';
                      }
                    }
                    if (isset($_GET['postID'])) {
                      $catID = $_GET['postID'];
                      $sql_2 = mysqli_query($conn, "SELECT * FROM `tb_game` WHERE cuid = '$catID'") or die(mysqli_error($conn));
                      $s2 = mysqli_fetch_array($sql_2);
                    }
                    ?>
                    <form role="form" action="<?= $urladmin; ?>/function/add-game.php" method="post" enctype="multipart/form-data">
                      ...
                      <div class="form-group mb-2">
                        <label class="form-label">Win Price :</label>
                        <input class="form-control" type="text" name="price" value="<?php if (isset($_GET['postID'])) {
                                                                                      echo $s2['price'];
                                                                                    } ?>" required>
                      </div>
                      <div class="form-group mb-2">
                      ...
                      <div class="form-group mb-2">
                        ...
                        <input class="form-control" type="text" name="diskon" value="<?php if (isset($_GET['postID'])) {
                                                                                        echo $s2['diskon'];
                                                                                      } ?>" required>
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Minimal Bet Amount :</label>
                        <input class="form-control" type="text" name="min_bet" value="<?php if (isset($_GET['postID'])) {
                                                                                        echo $s2['min_bet'];
                                                                                      } ?>" required>
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Maximal Bet Amount :</label>
                        <input class="form-control" type="text" name="max_bet" value="<?php if (isset($_GET['postID'])) {
                                                                                        echo $s2['max_bet'];
                                                                                      } ?>" required>
                      </div>
                      <div class="form-group mb-2">
                        ...
                        <select name="parent" class="form-control" required>
                          ...
                          <?php
                          $sql_21 = mysqli_query($conn, "SELECT * FROM `tb_game` WHERE parent = 0 ORDER BY cuid ASC") or die(mysqli_error($conn));
                          while ($s21 = mysqli_fetch_array($sql_21)) :
                          ?>
                            <option value="<?= $s21['cuid']; ?>" <?php if (isset($_GET['postID'])) {
                                                                    if ($s2['parent'] == $s21['cuid']) {
                                                                      echo ' selected=selected';
                                                                    }
                                                                  } ?>><?= $s21['title']; ?></option>
                          <?php endwhile; ?>
                      ...
                      <a href="<?= $urladmin; ?>/game/" class="btn btn-light">Cancel</a>
                    </form>
                  </div>
                </div>
              </div>
                    ...
                    <table id="default-datatable" class="invoice-list-table table border-top">
                      <thead>
                        <tr class="bg-primary">
                          ...
                          <th class="text-center">Min Bet</th>
                          <th class="text-center">Max Bet</th>
                      ...
                      <tbody>
                        <?php
                        $sql_1 = mysqli_query($conn, "SELECT * FROM `tb_game` ORDER BY cuid ASC") or die(mysqli_error($conn));
                        $no = 0;
                        while ($s1 = mysqli_fetch_array($sql_1)) :
                          $no++;
                          $idkategori = $s1['parent'];
                          $getSub = mysqli_query($conn, "SELECT * FROM `tb_game` WHERE cuid = '$idkategori'") or die(mysqli_error($conn));
                          $gs = mysqli_fetch_array($getSub);
                          if ($idkategori == 0) {
                            $parent = 'Root';
                          } else {
                            $parent = $gs['title'];
                        ...
                        ?>
                          <tr>
                            ...
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;"><?= $s1['title']; ?></td>
                            <td class="text-right" style="vertical-align: middle; white-space: normal; font-size: 14px; text-align: right;"><?= number_format($s1['min_bet'], 0, ",", "."); ?></td>
                            <td class="text-right" style="vertical-align: middle; white-space: normal; font-size: 14px; text-align: right;"><?= number_format($s1['max_bet'], 0, ",", "."); ?></td>
                            <td class="text-right" style="vertical-align: middle; white-space: normal; font-size: 14px; text-align: right;"><?= $s1['price']; ?> (<?php if ($s1['satuan'] == 0) {
                                                                                                                                                                    echo 'x';
                                                                                                                                                                  } else {
                                                                                                                                                                    echo '%';
                                                                                                                                                                  } ?>)</td>
                            <td class="text-right" style="vertical-align: middle; white-space: normal; font-size: 14px; text-align: right;"><?= $s1['diskon']; ?></td>
                            <td class="text-right" style="vertical-align: middle; white-space: normal; font-size: 14px; text-align: right;"><?= $parent; ?></td>
                            <td class="text-center" style="vertical-align: middle; font-size: 14px;">
                              <a href="<?= $urladmin; ?>/game/?postID=<?= $s1['cuid']; ?>" class="btn btn-primary btn-sm">
                                <i class="fa fa-edit"></i>
                              </a>
                              <?php if ($u['level'] == 'superadmin') : ?>
                                <a href="<?= $urladmin; ?>/function/del-game.php?cuid=<?= $s1['cuid']; ?>&jenis=1" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure want remove this data?');">
