        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4">
              <span class="text-muted fw-light">Pengaturan /</span> Slide Show
            </h4>
            <div class="row">
              <div class="col-sm-3">
                <div class="card">
                  <div class="card-body">
                    ...
                    if (isset($_GET['catID'])) {
                      $catID = $_GET['catID'];
                      $sql_2 = mysqli_query($conn, "SELECT * FROM `tb_slide` WHERE cuid = '$catID'") or die(mysqli_error($conn));
                      $s2 = mysqli_fetch_array($sql_2);
                    }
                    ?>
                    <form role="form" action="<?= $urladmin; ?>/function/add-slide.php" method="post" enctype="multipart/form-data">
                      <div class="form-group mb-2">
                        <label class="form-label">Upload Image :</label>
                        <input type="file" name="image" class="form-control">
                        <small>File jpg, jpeg, bmp, x-png, png, webp, gif</small><br>
                        <?php if (isset($_GET['catID'])) : ?>
                          <img src="<?= $domainutama; ?>/upload/slider/<?= $s2['image']; ?>" class="img-fluid">
                        <?php endif; ?>
                      </div>
                      <div class="form-group mb-2">
                      ...
                      <a href="<?= $urladmin; ?>/slide/" class="btn btn-light">Cancel</a>
                    </form>
                  </div>
                </div>
              </div>
                ...
                <div class="card">
                  <div class="card-datatable table-responsive">
                    <table id="default-datatable" class="invoice-list-table table border-top">
                      ...
                      <tbody>
                        <?php
                        $sql_1 = mysqli_query($conn, "SELECT * FROM `tb_slide` ORDER BY cuid ASC") or die(mysqli_error($conn));
                        $no = 0;
                        ...
                        ?>
                          <tr>
                            ...
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;"><img src="<?= $domainutama; ?>/upload/slider/<?= $s1['image']; ?>" style="display: block; margin: 0 auto; width: 250px; height: auto;"></td>
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;"><?= $s1['sort']; ?></td>
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;"><?php if ($s1['status'] == 0) {
                                                                                                                            echo 'Not Active';
                                                                                                                          } else {
                                                                                                                            echo 'Active';
                                                                                                                          } ?></td>
                            <td class="text-center" style="vertical-align: middle; font-size: 14px;">
                              <a href="<?= $urladmin; ?>/slide/?catID=<?= $s1['cuid']; ?>" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></a>
                              <a href="<?= $urladmin; ?>/function/del-slide.php?cuid=<?= $s1['cuid']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure want remove this data?');"><i class="fa fa-trash"></i></a>
                            </td>
                          </tr>
                        <?php endwhile; ?>
                      </tbody>
