...
<head>
  ...
  <?php include('../seo/meta/member-transaksi.php'); ?>
          ...
          <div class="col-lg-8 col-sm-12">
            <div class="card">
              <div class="card-header p-0">
                <ul class="nav">
                  <li class="nav-item">
                    <a class="nav-link active" href="<?= $urldomain; ?>/history"><i class="fa fa-bank" style="width: 20px; color: <?= $s0['warnadasar']; ?>;"></i> Banking</a>
                  </li>
                  <li class="nav-item">
              ...
              <?php if ($userLevel == 'user') { ?>
                <div class="card-body py-3 px-3">
                  ...
                  if (!empty($_GET['notif'])) {
                    ...
                    <span>Bukti pembayaran telah diupload, mohon ditunggu 1-5 menit kedepan.</span>
                    </div>
                    </div>
                    ';
                    }
                    ...
                    <div class="alert-message">
                    <span>Deposit pending.. Pembayaran dalam proses konfirmasi.</span>
                    </div>
                    </div>
                    ';
                    }
                    ...
                    <div class="alert-message">
                    <span>Withdraw berhasil! Proses penarikan membutuhkan waktu 2-10 menit.</span>
                    </div>
                    </div>
                    ';
                    }
                  ...
                  <ul class="list-group list-group-flush">
                    ...
                    $sql_transaksi = mysqli_query($conn, "SELECT * FROM `tb_transaksi` WHERE userID = '$IDuser' AND jenis IN(1,2) ORDER BY cuid DESC LIMIT 10") or die(mysqli_error($conn));
                        ...
                        <div class="row align-items-center" style="padding-top: 15px;">
                          ...
                          <div class="col-auto text-right">
                            ...
                            if ($st['jenis'] == 1) {
                              if ($st['status'] == 0) {
                                echo '
                              <a href="' . $urldomain . '/payment?trxID=' . $st['kd_transaksi'] . '" class="badge badge-primary text-white p-2">Menunggu</a>
                              <a onclick="alert(' . "'Deposit pending.. Pembayaran dalam proses konfirmasi.'" . ');" class="badge badge-primary p-2"><i class="fa fa-info-circle text-white"></i></a>
                              <a href="' . $urldomain . '/payment?trxID=' . $st['kd_transaksi'] . '" class="badge badge-primary p-2"><i class="fa fa-check-circle text-white"></i></a>
                              ';
                              } else if ($st['status'] == 1) {
                                echo '
                              <a href="' . $urldomain . '/payment?trxID=' . $st['kd_transaksi'] . '" class="badge badge-success text-white p-2">Berhasil</a>
                              <a onclick="alert(' . "'Deposit berhasil.. Dana telah ditambahkan ke akun Anda.'" . ');" class="badge badge-success p-2"><i class="fa fa-info-circle text-white"></i></a>
                              ';
                              } else if ($st['status'] == 2) {
                                echo '
                              <a href="' . $urldomain . '/payment?trxID=' . $st['kd_transaksi'] . '" class="badge badge-danger text-white p-2">Ditolak</a>
                              <a onclick="alert(' . "'Deposit ditolak.. Buat deposit baru jika telah transfer.'" . ');" class="badge badge-danger p-2"><i class="fa fa-info-circle text-white"></i></a>
                              ';
                              }
                            } else {
                              if ($st['status'] == 0) { //-withdraw
                                echo '
                              <a href="' . $urldomain . '/payment-withdraw?trxID=' . $st['kd_transaksi'] . '" class="badge badge-primary text-white p-2">Menunggu</a>
                              <a onclick="alert(' . "'Withdraw pending.. Dana akan segera masuk ke bank / emoney Anda.'" . ');" class="badge badge-primary p-2"><i class="fa fa-info-circle text-white"></i></a>
                              <a href="' . $urldomain . '/payment-withdraw?trxID=' . $st['kd_transaksi'] . '" class="badge badge-primary p-2"><i class="fa fa-check-circle text-white"></i></a>
                              ';
                              } else if ($st['status'] == 1) {
                                echo '
                              <a href="' . $urldomain . '/payment-withdraw?trxID=' . $st['kd_transaksi'] . '" class="badge badge-success text-white p-2">Berhasil</a>
                              <a onclick="alert(' . "'Withdraw berhasil.. Dana telah masuk ke bank / emoney Anda.'" . ');" class="badge badge-success p-2"><i class="fa fa-info-circle text-white"></i></a>
                              ';
                              } else if ($st['status'] == 2) {
                                echo '
                              <a href="' . $urldomain . '/payment-withdraw?trxID=' . $st['kd_transaksi'] . '" class="badge badge-danger text-white p-2">Ditolak</a>
                              <a onclick="alert(' . "'Withdraw ditolak.. Dana telah dikembalikan ke saldo akun Anda.'" . ');" class="badge badge-danger p-2"><i class="fa fa-info-circle text-white"></i></a>
                              ';
                              }
                            }
                            ?>
                  ...
                  $apanya = mysqli_query($conn, "SELECT * FROM `tb_transaksi` WHERE userID = '$IDuser' AND jenis IN(1,2) AND (`date` BETWEEN '$startMonth' AND '$startDay') ORDER BY cuid DESC") or die(mysqli_error($conn));
                  ...
                  ?>
                    <nav aria-label="Page navigation example">
                      <ul class="pagination justify-content-end">
                        ...
                        ?>
                          <li class="page-item<?= $link_active; ?>"><a class="page-link" href="<?= $urldomain; ?>/history?page=<?= $i; ?>"><?= $i; ?></a></li>
                        <?php endfor; ?>
                      </ul>
                    </nav>
                  <?php endif; ?>
