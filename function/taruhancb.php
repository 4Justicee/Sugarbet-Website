...
if ($saldoAktif < $subtotal) {
	header('location:' . $urldomain . '/taruhan?pid=' . $pid . '&gameid=5&notif=1');
	exit();
} else {
	for ($i = 0; $i < $jumlah; $i++) {
		$angka = $_POST['angka'][$i];
		...
		if ($nominal != '') {
			$insert_taruhan = mysqli_query($conn, "INSERT INTO `tb_taruhan` (`userID`, `pid`, `gameid`, `code`, `periode`, `created_date`, `tebak`, `posisi`, `nominal`, `menang`, `diskon`, `bayar`, `jumlah`, `keterangan`, `status`) VALUES ('$useridnya', '$pid', 5, '$kodenya', '$periode', '$created_date', '$angka', '$posisi', '$nominal', '$menang', '$diskon', '$totalBayar', '$jumlahMenang', '', 0)") or die(mysqli_error($conn));
