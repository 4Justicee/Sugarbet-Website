              ...
              <div class="col-sm-12">
                <div class="card">
                  <div class="card-body">
                    ...
                    $userID = $u['cuid'];
                    $sql_2 = mysqli_query($conn, "SELECT * FROM `tb_user` WHERE cuid = '$userID'") or die(mysqli_error($conn));
                    $s2 = mysqli_fetch_array($sql_2);
                    ?>
                    <form role="form" action="<?= $urladmin; ?>/function/edit-user.php" method="post" enctype="multipart/form-data">
                      <div class="form-group mb-2">
                        <label class="form-label">Upload Avatar :</label>
                        <input class="form-control" type="file" name="image">
                        <img src="<?= $domainutama; ?>/upload/avatar/<?= $s2['image']; ?>?<?= date('His') ?>" class="img-fluid rounded mt-1" style="max-height: 50px">
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Username :</label>
                        <input class="form-control" type="text" name="user" value="<?= $s2['user']; ?>" readonly>
                        <input class="form-control" type="hidden" name="postID" value="<?= $s2['cuid']; ?>" readonly>
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Password :</label>
                        <input class="form-control" type="password" name="pass" value="<?= $s2['re_pass']; ?>">
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Email Address :</label>
                        <input class="form-control" type="text" name="email" value="<?= $s2['email']; ?>" placeholder="Alamat Email">
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Full Name :</label>
                        <input class="form-control" type="text" name="full_name" value="<?= $s2['full_name']; ?>" placeholder="Nama Lengkap">
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Phone / Mobile Number :</label>
                        <input class="form-control" type="text" name="no_hp" value="<?= $s2['no_hp']; ?>" placeholder="No. Handphone">
                      </div>
