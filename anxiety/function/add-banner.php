...
$users = $u['user'];
$url_banner = mysqli_real_escape_string($conn, $_POST['url_banner']);
$ukuran_banner = mysqli_real_escape_string($conn, $_POST['ukuran_banner']);
...
if ($catID == '') {
    ...
    if ($_FILES['image']['size'] <= 2048000) {
        if ($gbr !== "" && $error == 0) {
            if (in_array(strtolower($tipe), $tipe_gambar)) {
                move_uploaded_file($_FILES['image']['tmp_name'], $upload_dir . $newname);
                $query = mysqli_query($conn, "UPDATE `tb_banner` SET `image` = '$newname', `url_banner` = '$url_banner', `ukuran_banner` = '$ukuran_banner' WHERE cuid = '$catID'") or die(mysqli_error($conn));
                header('location:../banner/?catID=' . $catID . '&notif=1');
            } else {
                header('location:../banner/?catID=' . $catID . '&notif=3');
            }
        } else {
            $query = mysqli_query($conn, "UPDATE `tb_banner` SET `url_banner` = '$url_banner', `ukuran_banner` = '$ukuran_banner' WHERE cuid = '$catID'") or die(mysqli_error($conn));
            header('location:../banner/?catID=' . $catID . '&notif=1');
        }
    } else if ($_FILES['image']['size'] >= 2048000 || $_FILES['image']['size'] == 0) {
