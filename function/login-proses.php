...
if (empty($user) && empty($pass)) {
	header('location:' . $urldomain . '/login?notif=1');
	exit();
} else if (empty($pass)) {
	header('location:' . $urldomain . '/login?notif=2');
	exit();
} else if (empty($user)) {
	header('location:' . $urldomain . '/login?notif=3');
	exit();
}
...
if (mysqli_num_rows($q) > 0) {
	...
	if ($statusnya == 1) {
		...
		unset($_SESSION['user']);
		session_destroy();
		header('location:' . $urldomain . '/login?notif=5');
		exit();
	}
} else {
	header('location:' . $urldomain . '/login?notif=3');
