              ...
              <div class="col-sm-12">
                <div class="card">
                  <div class="card-body">
                    ...
                    $sql_2 = mysqli_query($conn, "SELECT * FROM `tb_banner` WHERE cuid = 1") or die(mysqli_error($conn));
                    $s2 = mysqli_fetch_array($sql_2);
                    ?>
                    <form role="form" action="<?= $urladmin; ?>/function/banner.php" method="post" enctype="multipart/form-data">
                      <div class="form-group mb-2">
                        <label class="form-label">Upload Banner :</label>
                        <input type="file" name="image" class="form-control">
                        <small>File jpg, jpeg, bmp, x-png, png, webp, gif</small><br>
                        <?php if ($s2['image'] != '') : ?>
                          <img src="<?= $domainutama; ?>/upload/banner/<?= $s2['image']; ?>" class="img-fluid" style="max-height: 300px">
                        <?php endif; ?>
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Description :</label>
                        <textarea class="form-control summernoteEditor" type="text" name="deskripsi"><?= $s2['content']; ?></textarea>
                      </div>
                      <div class="form-group mb-2">
                        <label class="form-label">Status :</label>
                        <select name="status" class="form-control">
                          <option value="true" <?php if ($s2['status'] == 'true') {
                                                  echo 'selected = selected';
                                                } ?>> Active</option>
                          <option value="false" <?php if ($s2['status'] == 'false') {
                                                  echo 'selected = selected';
                                                } ?>> Not Active</option>
                        </select>
                      </div>
                      <button type="submit" name="submit" class="btn btn-primary">Publish</button>
                      <a href="<?= $urladmin; ?>/banner/" class="btn btn-light">Cancel</a>
                    </form>
                  </div>
                </div>
              </div>
