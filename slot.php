...
<head>
  ...
  <?php include('seo/meta/public-slot.php'); ?>
...
<body>
  ...
  <div id="wrapper">
    ...
    <h1 class="sr-only">Slot Pragmatic Play <?= $app_name; ?></h1>
    ...
    <div class="bg-custom pb-5">
      <div class="container">
        <div class="row">
          <?php
          $sql_3 = mysqli_query($conn, "SELECT * FROM `tb_gamenew` WHERE `game_category` = 'slots' ORDER BY RAND() LIMIT 54") or die(mysqli_error($conn));
          ...
          ?>
            <div class="col-sm-2 col-4 text-center pt-0 p-2">
              <a href="<?= $playUrl; ?>">
                <img data-sizes="auto" style="height: 120px;" data-src="<?= $urldomain; ?>/upload/game_pic/<?= $s3['game_provider']; ?>/<?= $s3['game_code']; ?>.png" src="<?= $urldomain; ?>/assets/images/loading.gif" class="lazyload shadow gamenormalimg" alt="Slot <?= $app_name; ?>">
