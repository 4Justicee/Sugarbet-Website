        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4">
              <span class="text-muted fw-light">Transaction /</span> Daftar Deposit
            </h4>
            ...
            <div class="card">
              <div class="card-datatable table-responsive">
                <table id="default-datatable" class="invoice-list-table table border-top">
                  ...
                  <tbody>
                    <?php
                    $sql_1 = mysqli_query($conn, "SELECT * FROM `tb_transaksi` WHERE jenis = 1 AND userID NOT IN (1,2) AND status = 1 ORDER BY cuid DESC") or die(mysqli_error($conn));
                    $no = 0;
                    while ($s1 = mysqli_fetch_array($sql_1)) :
                      ...
                      $IDuser = $s1['userID'];
                      $sql_5 = mysqli_query($conn, "SELECT * FROM `tb_user` WHERE cuid = '$IDuser'") or die(mysqli_error($conn));
                      $s2 = mysqli_fetch_array($sql_5);
                    ...
                    ?>
                      <tr>
                        ...
                        <td class="text-right" style="vertical-align: middle; white-space: normal; font-size: 14px;"><?= number_format($s1['total'], 0, ",", "."); ?></td>
                        <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;">
                          <?php if ($s1['note'] == '') {
                            echo "Tanpa Bukti & Catatan";
                          } else if ($s1['note'] == 'Bonus') {
                            echo $s1['transaksi'];
                          ...
                          <?php } else if ($s1['note'] != 'no-photo.webp' or $s1['note'] != 'no-photo.jpg') { ?>
                            <?php if ($domainutama == $urldomain) { ?>
                              <a href="<?= $urldomain; ?>/upload/bukti/<?= $s1['note']; ?>" class="btn btn-sm btn-primary" target="_blank">Bukti</a>
                            <?php } else { ?>
                              <a onclick="alert('Karna Deposit dari domain lain, untuk melihat bukti transfer cek nama web di Catatan (ex: bukti_namaweb_123.png), klik linknya dan ubah link gambar di address bar sesuai nama web pada domain lain. Hubungi developer jika belum paham.');" class="btn btn-sm btn-primary  text-white">Bukti</a>
