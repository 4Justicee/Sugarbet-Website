        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4">
              <span class="text-muted fw-light">Transaction /</span> Permintaan Penarikan
            </h4>
            ...
            <div class="card">
              <div class="card-datatable table-responsive">
                <table id="default-datatable" class="invoice-list-table table border-top">
                  ...
                  <tbody>
                    <?php
                    $sql_1 = mysqli_query($conn, "SELECT * FROM `tb_transaksi` WHERE jenis = 2 AND userID NOT IN (1,2) AND status = 0 ORDER BY cuid DESC") or die(mysqli_error($conn));
                    $no = 0;
                    while ($s1 = mysqli_fetch_array($sql_1)) :
                      ...
                      $sql_4 = mysqli_query($conn, "SELECT * FROM `tb_bank` WHERE userID = '$IDuser'") or die(mysqli_error($conn));
                      ...
                      if (stripos($metodenya5, ('CRYPTO')) !== FALSE) {
                        if (stripos($metodenya5, ('BTC')) !== FALSE or stripos($metodenya5, ('BITCOIN')) !== FALSE) {
                          $currency5 = 'BTC';
                          $totbar5 = $totaltransaksi5 / $ratewdbitcoin;
                          ...
                          $currency5 = 'ETH';
                          $totbar5 = $totaltransaksi5 / $ratewdethereum;
                          $totbarfix5 = number_format($totbar5, 8, ",", ".");
                          ...
                          $totbar5 = $totaltransaksi5 / $ratewdusdt;
                          $totbarfix5 = number_format($totbar5, 8, ",", ".");
                        ...
                        $currency5 = 'USD';
                        $totbar5 = $totaltransaksi5 / $ratewdusd;
                        $totbarfix5 = number_format($totbar5, 2, ",", ".");
                        $namapengirim = $s3['pemilik'];
                        $nomorpengirim = 'Email Paypal Penerima';
                        $namapenerima = $s4['pemilik'];
                        ...
                        $currency5 = 'IDR';
                        $totbar5 = $totaltransaksi5 / $ratewdidr;
                      ...
                      $notestatusdepo = 'Transfer Amount adalah Jumlah yang Ditransfer. Withdraw Amount adalah Saldo Member yang Dipotong. Pembayaran ini manual diproses dan ditransfer oleh admin ke member.';
                    ?>
                      <tr>
                        ...
                        <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;">
                          ...
                          <?php } else { ?>
                            <a href="<?= $urladmin; ?>/function/proses_withdraw.php?cuid=<?= $s1['cuid']; ?>" class="btn btn-primary btn-sm" onclick="return confirm('Are you sure want to Confirm this Transaction?');">Proses</a>
                            <a href="<?= $urladmin; ?>/function/reject_withdraw.php?cuid=<?= $s1['cuid']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure want to Reject this Transaction?');">Tolak</a>
                          <?php } ?>
                        </td>
                      </tr>
