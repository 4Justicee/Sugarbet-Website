        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4">
              <span class="text-muted fw-light">Togel /</span> Result
            </h4>
            <div class="row">
              <div class="col-sm-4">
                <div class="card">
                  <div class="card-body">
                      ...
                      if ($_GET['notif'] == 1) {
                        echo '
                              <div class="alert alert-success d-flex align-items-center" role="alert">
                                ...
                                <span><strong>Well Done!</strong> Result Saved!</span>
                              </div>
                            ';
                      }
                    }
                    if (isset($_GET['postID'])) {
                      $postID = $_GET['postID'];
                      $sql_2 = mysqli_query($conn, "SELECT * FROM `tb_periode` WHERE cuid = '$postID'") or die(mysqli_error($conn));
                      $s2 = mysqli_fetch_array($sql_2);
                    }
                    ?>
                    <form role="form" action="<?= $urladmin; ?>/function/add-result.php" method="post" enctype="multipart/form-data">
                      ...
                      <?php if (isset($_GET['postID'])) { ?>
                        <div class="form-group mb-2">
                          ...
                          <input class="form-control" type="text" name="periode" value="<?php if (isset($_GET['postID'])) {
                                                                                          echo $s2['no'];
                                                                                        } ?>" required>
                        </div>
                      <?php } else { ?>
                        <input class="form-control" type="hidden" name="periode" value="">
                      <?php } ?>
                      <div class="form-group mb-2">
                        <label class="form-label">Result :</label>
                        <input class="form-control" type="text" name="angka" value="<?php if (isset($_GET['postID'])) {
                                                                                      echo $s2['result'];
                                                                                    } ?>" required>
                      </div>
                      <button type="submit" name="submit" class="btn btn-primary">Publish</button>
                      <a href="<?= $urladmin; ?>/result/" class="btn btn-light">Cancel</a>
                    </form>
                  </div>
                </div>
              </div>
                    ...
                    <table id="default-datatable" class="invoice-list-table table border-top">
                      ...
                      <tbody>
                        ...
                        while ($s1 = mysqli_fetch_array($sql_1)) :
                          ...
                          $gp = mysqli_fetch_array($getPeriode);
                        ?>
                          <tr>
                            ...
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;"><?= $gp['no']; ?></td>
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;"><?= $gp['created']; ?></td>
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;"><?= $gp['result']; ?></td>
                            <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px;">
                              <a href="<?= $urladmin; ?>/result/?postID=<?= $gp['cuid']; ?>" class="btn btn-primary btn-sm">Edit</a>
                              <a href="<?= $urladmin; ?>/result_detail/?pid=<?= $pid; ?>" class="btn btn-success btn-sm">History</a>
                            </td>
                          </tr>
                        <?php endwhile; ?>
                      </tbody>
